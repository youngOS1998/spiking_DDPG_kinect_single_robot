...
SimpleLaserScan::SimpleLaserScan(ros::NodeHandle &n, int dir_num, std::string robot_name)
{
	...
	scan_sub_ = nh_.subscribe("/scan", 5, &SimpleLaserScan::scanCallback, this);
	dir_num_ = dir_num;
...
void SimpleLaserScan::scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg)
{
	...
	int dir_chunk_size = (int)((double)ranges_size / (double)dir_num_);   // 1440 points / 36
	int dir_chunk_start = (ranges_size - dir_chunk_size * dir_num_) / 2;  // (1440 - (1440/36)*36) / 2
	int median_pt = dir_chunk_size / 2;
	for(int dir=0; dir<dir_num_; dir++)  // dir_num_ : 36
	{
		...
		simple_data_[dir] = (double)ranges[tmp_chunk_middle];         // get the medians for each range
	}
}
...
int main(int argc, char** argv)
{
	...
	SimpleLaserScan *node = new SimpleLaserScan(n, 36, "mrobot");
	node->run(-1);
	return EXIT_SUCCESS;
}
