...
def train_ddpg(run_name="DDPG_R1", exp_name="Rand_R1", episode_num=(300, 400, 500, 600),
               ...
               state_num=4, action_num=2, is_pos_neg=False, is_poisson=False, poisson_win=50,
    ...
    env_episode = 0
    env_ita = 0
    ita_per_episode = iteration_num_start[env_ita]       # iteration_num_start=(200, 300, 400, 500)
    env.set_new_environment(overall_init_list[env_ita],  # init_pose_list
                            overall_goal_list[env_ita],  # goal_list
                            overall_poly_list[env_ita])  # obstacle_list
    ...
    while True:
        # [list(1x4), np.array(1x480x640)]
        state = env.reset(env_episode)  # state = [tmp_robot_pose, tmp_robot_spd, tmp_robot_scan]: tmp_robot_pose -> [msg.pose[-1].position.x, msg.pose[-1].position.y, yaw]
                                        #                                                        : tmp_robot_spd  -> [sqrt(x**2 + y**2), msg.twist[-1].angular.z]
                                        #                                                        : tmp_robot_scan -> np.array(* x 36)
        if is_pos_neg:
            rescale_state = ddpg_state_2_spike_value_state(state, rescale_state_num)
        else:
            rescale_state = ddpg_state_rescale(state, rescale_state_num)   # [list(1x4), np.array(1x480x640)]
        episode_reward = 0
        for ita in range(ita_per_episode):  # 200
            ita_time_start = time.time()
            overall_steps += 1
            raw_action = agent.act(rescale_state)
            # decode_action = network_2_robot_action_decoder(
            #     raw_action, 0.1, 0.5, 0.1, 0.5
            # )
            decode_action = wheeled_network_2_robot_action_decoder(
                raw_action, linear_spd_max, linear_spd_min
            )
            next_state, reward, done = env.step(decode_action)   # next_state: DDPG state
            if is_pos_neg:
