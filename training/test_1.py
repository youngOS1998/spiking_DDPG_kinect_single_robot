...
NEURON_VTH = 0.5
# NEURON_CDECAY = torch.Tensor(0.5).to(torch.device('cuda'))
# NEURON_VDECAY = torch.Tensor(0.75).to(torch.device('cuda'))
NEURON_CDECAY = 0.5
NEURON_VDECAY = 0.75
...
class CriticNetSpiking(nn.Module): 
    ...
    def forward(self, x):
        ...
        data_in = torch.cat([feature_pos, feature_neg, normal_state, action_state], 1)
...
if __name__ == "__main__":
    ...
    actor_net = ActorNetSpiking(combined_num=9600, action_num=2, device=device).to(device)
    with torch.no_grad():
        spikes_value = np.random.rand(1, 6, 480, 640)
        data_spikes = np.random.rand()
    ...
    #       combined_data = [state_spikes_pos, state_spikes_neg]
